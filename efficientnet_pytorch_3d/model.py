...
class MBConvBlock3D(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        self.is_group_norm = global_params.is_group_norm
        ...
        if self._block_args.expand_ratio != 1:
            ...
            self._gn0 = nn.GroupNorm(num_groups=global_params.group_num)
        ...
        self._gn1 = nn.GroupNorm(num_groups=global_params.group_num)
        ...
        self._gn2 = nn.GroupNorm(num_groups=global_params.group_num)
...
class EfficientNet3D(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, in_channels=3):
        ...
        self._gn0 = nn.GroupNorm(num_groups=self._global_params.group_num)
        ...
        self._gn1 = nn.GroupNorm(num_groups=self._global_params.group_num)
    ...
    def extract_features(self, inputs):
        ...
        # Stem
        if self._global_params.is_group_norm:
    ...
    # @classmethodに関して、インスタンスメソッド（インスタンス後にしか利用できない関数）とは異なり
    # インスタンスに紐づかないメソッドである。そのためインスタンスを生成しなくても実行できる。
    @classmethod
